    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        <div class="layui-inline">
          <label class="layui-form-label">报考项目</label>
          <div class="layui-input-inline">
            <input type="text" name="baokaoxiangmu" id="baokaoxiangmu" placeholder="报考项目" autocomplete="off" class="layui-input">
          </div>
        </div>   
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('zaixianbaoming','新增')" @click="jump('../zaixianbaoming/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="proBox" v-for="(item,index) in dataList" v-bind:key="index">
        <div class="proBoximg">
          <div class="primg" style="background-size: cover;background-repeat: no-repeat;background-position: center;">
            <img :src="item.xiangguantupian?item.xiangguantupian.split(',')[0]:''" width="390" height="320" />
          </div>
        </div>
        <div class="proBoxtxt">
          <p>{{item.baokaoxiangmu}}&nbsp;&nbsp;<span style="color: red;" v-if="item.price">{{item.price}} RMB</span></p>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#baokaoxiangmu').val()) {
          param['baokaoxiangmu'] = jquery('#baokaoxiangmu').val() ? '%' + jquery('#baokaoxiangmu').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('zaixianbaoming/list', 'get', param, function(res) {
